...
void main()
{
  ...
  uvec3 triangleIndices = uvec3(INBARY[0].vertexID, INBARY[1].vertexID, INBARY[2].vertexID);
    ...
    else
    {
      ...
      // Snap the shading normal into the hemisphere of the viewer-facing geometric normal. The
      // screen-space derivative normal always points toward the camera, so this puts wNormal on the
      // visible side regardless of the vertex-normal orientation. Handles all of: mirrored (negative
      // determinant) instances, where the inverse-transpose flips the normal to the wrong side; the
      // back side of two-sided geometry; and the mesh-shader winding swap. Matches the geometric-side
      // flip the path tracer applies via backFacing.
      vec3 wGeoNormal = -cross(dFdx(IN.wPos), dFdy(IN.wPos));
