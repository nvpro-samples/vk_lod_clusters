# BLAS Caching for ray tracing cluster-based continuous level of detail
...
a discrete LoD level. Therefore the cached BLAS is based on a single discrete LoD level.
...
The streaming system uses the existing scene level patching to provide information about cached BLAS updates and builds.

![image presenting the list of device operations](blas_caching_device.png)

On the device we do some BLAS build preparation, gathering the CLAS (Cluster Level Acceleration Structure) addresses used for the cached BLAS.
...
* The sample did not build cached BLAS directly into the persistent storage, but copies them from the per-frame implicit BLAS location.
